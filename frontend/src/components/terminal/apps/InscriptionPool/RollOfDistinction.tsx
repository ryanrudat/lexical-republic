...
// parent .crt-phosphor-monitor. No card chrome — just green text.
  ...
  return (
    <section>
      ...
      {loading && roll.length === 0 ? (
        <p className="phosphor-text-dim text-[12px] uppercase tracking-[0.3em] text-center py-3">
          loading...
        </p>
      ) : roll.length === 0 ? (
        <p className="phosphor-text-dim text-[12px] uppercase tracking-[0.3em] text-center py-3">
          ...
          {roll.map((r, i) => {
            ...
            return (
              <li key={`${r.citizenNumber}-${i}`} className={`flex items-center gap-4 ${colorClass}`}>
                ...
                <span className="text-[12px] tabular-nums tracking-wider phosphor-text-dim">
                  P.I. {r.piTotal}
                </span>
              </li>
            );
      ...
      <p className="phosphor-text-faint text-[11px] uppercase tracking-[0.3em] italic text-center mt-6">
