...
- **Attempt ≥3 with an off-topic history RE-EVALUATES the current text** — a fresh on-topic verdict auto-passes (vocabScore floored 0.3). Never replay a stale off-topic verdict; that was a permanent lock violating pedagogy §5.5/§1.5.
...
- **Forward `isDegraded` into onComplete details** (ShiftReport/PriorityBriefing do) — the teacher must distinguish "AI never saw this" from a genuine 0.3. `submitAnyway` floors vocabScore via `Math.max(0.3, …)` — `?? 0.3` doesn't catch the degraded path's 0.
