...
- **ShiftQueue emits the initial task list/label via `once('connect')`** (mount effect) — a cold-open socket would otherwise drop the emit permanently and leave the teacher stuck on "Loading task info…". Mirrors the ClarityQueueApp enter-shift pattern.
