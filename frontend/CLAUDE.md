...
- **PrioritySort justify card re-displays the student's filed folder (added 2026-06-11)** — derived from cascade `columns` state (`COLUMNS.find(col => columns[col].includes(caseId))`), rendered as the compact `Folder` glyph + colored label on the case card, and the chosen folder is also named in the `taskContext` sent to WritingEvaluator (the on-topic veto judges against the student's actual decision). Don't remove the reminder: A2 students can't recall six filings across the phase boundary (pedagogy.md §7.7.1 "justify-phase referent visibility"). `modalLane` in week3.ts is config-only — NOT consumed by the component.

## Shift Closing & Grade Registration (added 2026-06-30)
- **`ShiftClosing` posts the canonical `ShiftResult` (closing grade) on mount** via `postShiftResult`. That POST now **retries 3× with backoff**; the additive `patchConcern` stays OUTSIDE the retry loop (it must run at most once); on total failure `hasPosted.current` resets so re-entering the completed shift re-attempts. The old single fail-silent POST lost grades on a dropped request (classroom Wi-Fi / transient 5xx). The backend ALSO lazily ensures a `ShiftResult` when the week's final task completes (server convergence), so this client POST is the richer refinement, not the only path. **W4 caveat**: `ShiftClosing` is gated behind the Drop Box → Recruitment epilogue (`ShiftQueue.tsx`, `weekNumber === 4`); convergence is what guarantees the grade registers even if a student abandons that epilogue.
- **Gradebook CSV export** (`Gradebook.tsx`): per-student "⤓ CSV" button (student-name cell) + header "⤓ Download all (CSV)". `buildGradeRows` reuses the module-scope `computeCell` so the CSV's overall %/status/labels match the grid exactly; `downloadGradeCsv` builds a Blob (leading UTF-8 BOM) → `URL.createObjectURL` → programmatic `<a download>`. **Do NOT switch to an `<a href>` pointing at an API route** — auth is a Bearer token injected by the axios interceptor, which a browser-initiated request can't carry (same reason `<video>` src must hit an unauthenticated static path). All data is already in the `/teacher/gradebook` payload; no new endpoint.
...
- **Spy surfaces hide during PEARL lockouts (added 2026-06-11)** — `body.compliance-check-active` / `body.clarity-check-active` (ClarityCheck now adds the latter) `display:none` the `.edited-pill` + `.edited-window`. Needed because the spy UI mounts at App root AFTER the Routes subtree: EditedWindow's z-[90] tied with ClarityCheck's z-[90] and won by DOM order.
- **`FreyIntroOverlay` (added 2026-06-11)** — one-time first-contact note (Frey register) on the terminal desktop once W4 unlocks, explaining the Records Wing tile + the corner `[ ]`. Gated by `spyStore.introSeen` ← NarrativeChoice `w4_funnel_intro`='seen' (fail-open write, like all spy writes). Desktop-only (`terminalApp === 'desktop'`) so it can never collide with shift lockouts; `body.frey-intro-active` hides the PEARL island. Copy = `FREY_FIRST_CONTACT` in spyFiles.ts.
- **Dark leads reopen after `SNOOP_RETRY_COOLDOWN_MS` (10 min, added 2026-06-11)** — `spyStore.darkAt` records when each lead went dark (from the row's `createdAt` on load, so leads dark before this change are retryable immediately). `startExtract` is the authority: it clears an expired dark entry and re-rolls; `funneled` is permanent. `DarkLead` in RecordsRoomApp shows a countdown (30s ticker) then the retry button. A retried outcome writes a newer `w4_snoop_*` row — `loadChoices` is last-write-wins per key.
...
## Session / Auth Lifecycle (added 2026-06-09, extended 2026-06-10)
- **`resetSessionStores()` in `studentStore.ts` is THE session-teardown cascade** — pairs share a Chromebook and student B logs in after A with **no page reload**, so any store not cleared bleeds across (open remediation modal, private dictionary notes, wrong narrative route, stuck pause overlay, unread message toast, Harmony credits under A's localStorage key, an in-flight Word Pool drill, PEARL's "allocation exhausted" lock). It resets: spy, session (`resetConcern` — also clears latent `attemptCounts`/`lastGrammarError`), shiftQueue, shift, season, dictionary, messaging (incl. the module-level `inFlightKeys` Set — its keys carry no pairId), harmony, inscription, sessionPause, audio, pearl (full `reset()` — `clearChat` alone left `barkHistory` rendering A's bark log to B in PearlPanel/MessagesApp, plus undismissed announcements). **Any new session-scoped store needs a `reset()` AND a call in `resetSessionStores()`.** `login()` only re-hydrates concern score — it does NOT clear the others.
- **`refresh()` (the focus/visibility re-validation) clears the session ONLY on 401/403** — a transient network error must keep the user, or Chromebook wake-from-sleep (when Wi-Fi is most likely down) logs students out mid-shift and drops unsaved input. On genuine auth failure it runs `disconnectSocket()` + the SAME `resetSessionStores()` cascade as logout (the next login may be a different student).
- **`sessionStore.resetRateMachine()` vs `resetConcern()`**: `resetConcern()` zeroes the DB-backed `concernScore` too (use on logout). `resetRateMachine()` clears stage/buffer/timers/`activeRemediation`/`inscriptionDrillActive` but PRESERVES the score (use on teacher shift transfer — the score must persist across the move).
- **Tasks read the student's lane from `useStudentStore(s => s.user?.lane ?? 2)`, NOT from `config.lane`** — `document_review` configs never set a top-level `lane`, so the old `config.lane ?? 2` hardcoded Lane 2 and Lane-1 students lost their `ErrorCorrectionDoc` hints. `laneHints["1"]` MUST have one entry per error in `errors[]` order (the renderer indexes `laneHints["1"][errorIndex]`).
- **`shiftQueueStore.loadWeekConfig` zeroes `concernScoreDelta`/`concernScorePersisted` when entering a different shift** — a student exiting a completed shift via the terminal X/Home and re-entering the next one bypasses `reset()`, so without this the prior shift's concern double-counts into the new ShiftClosing.
...
## Harmony Engagement Overhaul — M0–M3 (added 2026-05-29)
- **Verdict loop lives in `HarmonyApp.tsx`** — `VerdictControls` + `FlagModal` render for `post.postType === 'feed_review'` only. The flag modal reads active rules from `activeRulesForWeek(week)` (W1: `reg_14c`+`conduct_sentiment`; W2+: adds `conduct_s1`) and chips from `post.flagOptions`. `submitVerdict` (harmonyStore) hits `POST /harmony/posts/:id/verdict`, updates the post in `posts` (so the card re-renders answered) and awards `VERDICT_CREDIT` (3) HC on correct. After answering, the answer key (`correctVerdict`/`violations`) is present (server only sends it post-answer); `buildClientReaction` reconstructs the PEARL line on reload.
- **Feed drip is INDEX-based — never store post objects in drip state.** `FeedTab` computes `displayed = posts.slice(Math.max(0, holdCount - revealedCount))` from the CURRENT posts each render, and the drip effect resets ONLY on `postIdsKey` change (the joined id list), NOT on `posts` identity. This is why filing a verdict (which mutates a post in place) no longer reshuffles/“jumps” the board. Cadence: first reveal 15–40s, then 30–75s; `holdCount` ≤ 3. The "↑ N NEW POST" pill scroll-to-tops + clears `newCount`.
- **Harmony Credits (HC)** persist in `localStorage` keyed `harmony_credits_${pairId}` (`loadCredits` on mount, `awardCredits` writes through). Keyed per pair → no shared-device leak. Shown in `HarmonyHeader`.
- **Daily Vocabulary Audit** (`DailyVocabAudit`) renders at top of Feed from `auditPairs` (backend `/posts`). Match game; gated once-per-shift via `localStorage` `harmony_audit_${pairId}_w${week}`; awards HC; collapses to a "complete" chip.
- **PEARL presence uses the `pearl-eye-glow.png` image (`PearlGlyph`), NOT the RAF `PearlEye` component** inside Harmony — lighter, no store coupling, brand-correct. New atmosphere CSS (`.harmony-ticker`, `.harmony-4488-glitch`) is in `index.css` and auto-neutralised by the global `prefers-reduced-motion` rule. No CRT/green inside Harmony (cream/sky palette only).
- **`class:presence` socket listener** is registered in `App.tsx` (not `useTeacherSocket`) → `harmonyStore.setClassOnline`. HarmonyApp emits `student:presence-request` on mount for an immediate count.

## Teacher Real-Time ClassMonitor (added 2026-05-29)
- **`useTeacherSocket` binds all three remediation events** (`-fired` → `incrementRemediation`; `-completed`/`-clawback` → `setLiveConcern` + `flagRemediationClawback`). Add new store actions to the effect deps array or stale-closure bugs creep in.
- **`teacherStore` carries `remediationClawback` + `liveConcern`** (both keyed by pairId). ClassMonitor derives `clawedBack` + `concern = liveConcern[id] ?? student.concernScore` and renders a "↩ clawed back" badge + a threshold-colored "Concern: X.X" chip. `StudentSummary.concernScore` is populated by `GET /students`.
- **ClassMonitor `now`-tick is 5s** (was 30s) — a pure re-render decoupled from the 30s data poll, so time-driven struggle flags + elapsed timers track the threshold closely with no extra server load.
- **Student-card grid uses `items-start`** so expanding one card doesn't stretch its row-mates (default `align-items: stretch` did). Don't give the card `h-full`.
- **ShiftQueue announces the task list/label via a PERSISTENT `on('connect')` handler** (`emitCurrent`, reads live store state) — covers both the cold-open handshake and every reconnect (see Socket Reconnect Re-emits below). The old `once('connect')` form left the teacher card hollow after any >5s drop.
- **WritingEvaluator clears its progress sub-line UNCONDITIONALLY on unmount** (`getSocket()?.emit(...)`, no `connected` guard) — the guard could drop the clear during a reconnect, leaving a stale "Writing: N words" on the teacher card.

## Loader Staleness Epochs (added 2026-06-10/11)
- **Every async store loader carries a module-level `loadEpoch`**: `const epoch = ++loadEpoch` at entry, `if (epoch !== loadEpoch) return` after EVERY await (including in the `catch`, or a stale rejection stamps an error onto fresh state), and `loadEpoch++` as the first line of `reset()`. Applied to: shiftQueueStore.loadWeekConfig, shiftStore.loadWeek, dictionaryStore.loadDictionary/loadFamilies (families SNAPSHOTS without bumping — it runs back-to-back with loadDictionary), seasonStore.loadSeason, messagingStore.loadMessages, spyStore.loadChoices.
- **Why**: stores are module-global, so a fetch that outlives logout writes student A's data back into B's cleared store — and the `length === 0` mount gates then suppress B's refetch forever. Same epoch invalidates a previous shift's in-flight config fetch after a teacher transfer (wrong-week MissionScore writes). Any NEW store loader must follow this pattern.
- **completeTask returns `'persisted' | 'stale-dropped'`** — its post-await freshness guard (taskProgress reference identity + weekNumber) resolves `'stale-dropped'` when a teacher command rewrote progress mid-POST; a network failure THROWS. ShiftQueue.handleComplete catches the throw into the rose "TRANSMISSION INTERRUPTED — ↻ Resubmit to Bureau" retry banner and silently stops the cascade on stale drops. The two exits must never share a path.

## Socket Reconnect Re-emits (added 2026-06-10/11)
- **Connect-time announcements use persistent `sock.on('connect', handler)` with effect cleanup — NEVER `once('connect')`.** After any >5s disconnect the server rebuilds the student's status HOLLOW (no taskId/tasks), so each surface re-asserts on every reconnect: ClarityQueueApp (enter-shift + change-step), ShiftQueue (`emitCurrent` — reads the LIVE task from the store, not the mount closure), the parked-state effect (gated/shift-complete), InscriptionDrill (enter-drill — server side also rejoins the lobby room + replays paused/resumed), and App.tsx (Open Pool queue re-join via `inscriptionStore.queueWeekNumber`).
- **teacherStore.mergeHollowStatus** — `upsertStudent`/`setClassSnapshot` merge a hollow rebuilt status with the prior rich record instead of blanking the teacher's card (and only then delete the lastKnown fallback).
- **Teacher-monitor emits never live in render bodies** — the gated/shift-complete emits are a transition-keyed `useEffect`; render-phase emits re-fired a staff-room broadcast on every re-render.

## WritingEvaluator Contract (added 2026-06-10/11)
- **`onTopic: data.onTopic !== false`** (fail-open) — `=== true` coerced any response missing the field into an off-topic accusation that hid Submit Anyway. Backend Layer-1 fails now send `onTopic: true`, but keep the frontend fail-open (services deploy independently).
- **Grading is DETERMINISTIC + one pass rule for every attempt (2026-06-22)** — backend grades at `temperature: 0`, and the frontend computes `passed = onTopic && meetsWords && vocabScore >= 0.3` (plus an attempt-3 on-topic auto-pass), the SAME way on every attempt. This replaced the attempt-1-vs-2-vs-3 ladder, which (with the 0.7-temperature grader) made the same draft pass/fail at random. Don't re-introduce per-attempt bars.
- **One nudge, then escapable (2026-06-22)** — on-topic drafts can Submit Anyway right after the first fail; an off-topic draft gets one nudge, then Submit Anyway opens on `attempt >= 3` **even if still off-topic**. `submitAnyway` PRESERVES the real `onTopic` verdict + `submittedAnyway` (teacher sees the truth) and floors vocabScore 0.3 on-topic / 0.2 off-topic. Vocab is scored, not gated (no 30%-of-targets wall). Don't re-add a hard off-topic lock.
- **Pass `minWords` per task** — it flows to the backend as `metadata.minWordCount` and replaces the hidden 30/40-word lane formula. PrioritySort uses 10 (matching its "1-2 sentences" prompt) AND keys the evaluator per justify case so attempts reset.
- **Pass `writingPrompt` + `taskContext`** or the AI rubric defaults `onTopic=true` and the veto is structurally off (this bit PriorityBriefing + PrioritySort).
- **Pass `missionId`** (resolved by ShiftQueue via missionType, forwarded through `TaskProps`) — it enables the server-side eval + pearlFeedback persistence that was dead code without it. PrioritySort deliberately NOT wired (per-case evals would churn one row).
- **Forward `isDegraded` into onComplete details** (ShiftReport/PriorityBriefing do) — the teacher must distinguish "AI never saw this" from a genuine 0.3. `submitAnyway` floors vocabScore via `Math.max(floor, …)` (floor = 0.3 on-topic / 0.2 off-topic) — `?? floor` doesn't catch the degraded path's 0.
