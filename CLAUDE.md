...
- **Don't-break infra**: per-class socket scoping only (`class:${classId}` + `teacher:${teacherId}`; the global `'teacher'` room is GONE) · `MissionScore.details` must be read-and-merged inside a `$transaction` before writing (never clobber) · `JWT_SECRET` is mandatory at module load · swapping a `public/` image needs a filename rename to cache-bust.
