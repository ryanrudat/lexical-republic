...
## 2026-07-01 (shift closing-grade registration reliability + Gradebook CSV export)

Teacher-reported: some students who completed a shift never had the grade register on the teacher view — the "closing shift" never registered. Root-caused via a multi-agent trace. There is no single "close shift" flag: the teacher view treats a shift as registered when EITHER the `ShiftResult` row (aggregate closing grade, written only by the frontend `ShiftClosing` screen) OR the week's final-task MissionScore (`shift_report`/`clock_out`, `status:'complete'`) exists. The `ShiftResult` write sat in a fail-silently `try/catch` with no retry, so a dropped POST (classroom Wi-Fi / transient 5xx) — or never reaching `ShiftClosing` at all (the W4 Drop Box → Recruitment epilogue gates it) — left students on a normal "Shift Complete" screen with no grade registered.

Fix (3 commits to master; frontend build + backend tsc + 55 backend tests pass):
- `750858a` — (1) `ShiftClosing.tsx`: bounded 3-attempt retry on `postShiftResult`; the additive `patchConcern` kept OUTSIDE the retry loop (must run at most once); `hasPosted` reset on total failure so re-entry re-attempts. (2) **Server-side convergence** — new `backend/src/utils/shiftResultRegistration.ts::ensureShiftResultRegistered`, called from the `/shifts/.../score` endpoint the moment the closing task is marked complete; lazily ensures a `ShiftResult` exists (server aggregate mirrors `frontend/src/utils/scoreAggregator.ts`), never clobbers a richer client write, flips a `completedAt:null` marker to complete, omits `concernScoreDelta`, non-fatal after the score commits. (3) **Backfill script** `backend/src/scripts/backfillShiftResults.ts` (dry-run default, `--commit`) recovers already-stuck students.
- `ca41fad` — generalised convergence + backfill to key on the **week's final config task** (`getClosingTaskType`), NOT a hardcoded `shift_report`/`clock_out` list, so **Shift 2** (closes on `contradiction_report`) is covered too. Validated: W1/W3/W4 close on `shift_report`, W2 on `contradiction_report`.
- `5188b2c` — **Gradebook CSV export**: per-student "⤓ CSV" button (in the student-name cell) + header "⤓ Download all (CSV)". One row per task across every shift (task label + score% + status) plus a "SHIFT OVERALL" row, mirroring the grid via `computeCell`. Built entirely client-side from the already-loaded `/teacher/gradebook` payload (no new endpoint); Blob → object-URL → `<a download>` (a plain `<a href>` can't carry the Bearer token); UTF-8 BOM for Excel.

Backfill run against prod is pending (run `npx tsx src/scripts/backfillShiftResults.ts` dry-run, then `--commit` with the prod `DATABASE_URL`). Known remaining: "Symptom A" students who never finished the closing task have no grade to register; `ShiftResult` is pair-only (legacy User scores keep the MissionScore fallback); the teacher-view path-(b) fallback at `teacher.ts:237` is still hardcoded to `shift_report`/`clock_out`, but convergence now writes the `ShiftResult` so W2 shows via path (a).

---

## 2026-06-22 (open-writing grading stabilisation + doctrine doc sweep)

Teacher-reported: the Shift Report writing grading was "hairy," unpredictable, and too strict — students couldn't reliably finish. Root causes traced end-to-end: (1) the AI grader ran at `temperature: 0.7`, so the *same* draft was graded differently each resubmit; (2) one AI "off-topic" verdict hard-locked the student (score 0, Submit Anyway disabled, no auto-pass); (3) the pass bar changed every attempt (1 used the backend verdict, 2 lowered the vocab bar, 3 auto-passed); (4) scattered gates with different numbers (half-words to submit, full to escape, a backend word floor + a 30%-of-targets vocab floor + a vocab-score floor). User chose **"one nudge, then escapable."**

Fix (builds pass): `backend/src/routes/submissions.ts` — grader → `temperature: 0` (deterministic); on-topic rubric §1 rewritten to **default-true / lenient** (only clearly-unrelated or gibberish is off-topic); pass floor 0.4 → 0.3; the **30%-of-targets vocab Layer-1 hard block REMOVED** (vocab is the scored axis, not a gate — low vocab now yields a low score + a "use more target words — try: X" hint). `frontend/.../WritingEvaluator.tsx` — collapsed the attempt-1/2/3 ladder into **one rule** (`onTopic && meetsWords && vocabScore >= 0.3` + attempt-3 on-topic auto-pass); **off-topic is now escapable** (first verdict shows a nudge, after one revise Submit Anyway opens even if still off-topic, preserving `onTopic:false` + `submittedAnyway` for the teacher); crisp plain-English action hints replace in-world-only failure prose. Applies to **all** writing surfaces (shift report every week, PriorityBriefing, PrioritySort — shared evaluator).

Deliberately softens the "strict on-topic veto" locked decision (user-directed). Doc sweep updated `CLAUDE.md`, `frontend/CLAUDE.md`, `backend/CLAUDE.md`, `docs/pedagogy.md` §5.1/§5.5 + principle 9 + summary table, `docs/locked-decisions.md`, `docs/features.md`. Memory `feedback_writing_grading_one_nudge_escapable` records the decision so it isn't reverted; the "no grammar scoring on open writing" rule is UNCHANGED.

---

## 2026-06-18 (Shift 4 polish — readable observation log, Evidence Board vocab skin, Look-back recall, blocking Clarity Minder)

Four commits, pushed. (1) `57291f8` — W4 Activity Reconciliation Desk: the Daily Observation Set comprehension screen rendered its surveillance log as a run-on wall of text (authored `\n`s collapsed inside `DocumentCard`'s single `<p>`). `ComprehensionDoc` now renders a structured, scannable log (one aligned row per observation) when the doc carries an `observations[]` array, plus an in-world Directions banner (config-driven) + a line-by-line reveal. Prose comprehension docs (W1 memo) unchanged. (2) `79c758d` — W4 Vocabulary Clearance "Evidence Board" case-file skin: per-shift theming keyed on `weekConfig.weekNumber` (warm manila surface, evidence-tag header, lettered "exhibits", chain-of-custody progress, "✓ CLEARED" stamp). Mechanic untouched; W1–3 keep the default look. (3) `7e0bd20` — W4 Shift Report "Look back at your shift" recall scaffold: collapsible, config-driven (`recall` block), lists what the student did this shift grouped First/Next/Finally (notes, not copyable sentences). (4) `c1f7ddc` — Clarity Minder direct messages now BLOCK the screen: dedicated `clarityAlert` store slot + `ClarityMinderAlert` modal at App root (`z-[1200]`, above all overlays); Acknowledge/Reply; NPC barks keep the ambient corner toast.
