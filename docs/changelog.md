...
- ~23 unverified finder claims (chunk 2+ awaiting approval) — top remaining: shift_end pending-ref clobber, task-completion failure path, loadWeekConfig cancellation token, writing mastery bump not veto-gated, degraded-path auto-pass, dead missionId/pearlFeedback persistence, teacher-bound socket events readable by students.
