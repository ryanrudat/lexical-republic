...
- **Submit Anyway + one-nudge-then-escapable** (2026-04-24, PR #15; reworked 2026-06-22): `WritingEvaluator` shows an amber "Submit Anyway" button after 1 failed attempt, gated on the task's `minWords`. **On-topic** drafts escape immediately; **off-topic** drafts get one nudge, then escape on attempt 3 (preserving the off-topic flag for the teacher). Fires `onResult({ passed: true, submittedAnyway: true })` with `vocabScore` floored (0.3 on-topic / 0.2 off-topic). `ShiftReport.handleResult` records the clamped `[0.1, 1.0]` **vocab-only** score (grammar removed from scoring in the 2026-04-29 rubric redesign). The grader is deterministic (`temperature: 0`). Students never get stuck at ShiftReport; the shift always advances to ShiftClosing.
...
- **Grade breakdown CSV export (added 2026-07-01)**: per-student "⤓ CSV" button in the Gradebook student-name cell downloads that student's per-shift, per-task breakdown (columns: Student, Designation, Shift, Shift Title, Task, Score, Status, Completed — one row per task plus a "SHIFT OVERALL" row per shift); a header "⤓ Download all (CSV)" exports every student in one file. Built entirely client-side from the already-loaded `/teacher/gradebook` payload (no new endpoint) — task labels + overall % reuse `computeCell` so the CSV matches the on-screen grid exactly. Blob → object-URL → `<a download>` (a plain `<a href>` to an API route can't carry the Bearer token); UTF-8 BOM so Excel reads non-ASCII names. `frontend/src/components/teacher/Gradebook.tsx`.
...
- **AI evaluation**: Prompt includes lane context; deterministic (`temperature: 0`); single pass rule `onTopic && meetsWords && vocabScore >= 0.3` across all tiers (vocab-only — grammar not scored; one-nudge-then-escapable on off-topic; 2026-06-22)
