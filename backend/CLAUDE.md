...
- **Layer-1 writing fails carry `onTopic: true`** (a length/vocab verdict is not a topic verdict) and `/evaluate` honors `metadata.minWordCount` (lane-resolved per-task floor, clamped 5-100) over the legacy lane formula.
