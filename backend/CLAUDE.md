...
## Shift Result Registration — closing-grade convergence (added 2026-06-30)
- **A shift's closing grade registers via TWO independent writes; the teacher view treats EITHER as complete**: the `ShiftResult` row (aggregate, written by the frontend `ShiftClosing` POST) OR the week's final-task MissionScore (`status:'complete'`, counted only for `shift_report`/`clock_out` at `teacher.ts:237`).
- **Server-side convergence**: `backend/src/utils/shiftResultRegistration.ts::ensureShiftResultRegistered({ pairId, weekNumber, completedAt? })` is called from the `/shifts/.../score` endpoint the moment the **week's FINAL config task** is marked complete — keyed via `getClosingTaskType(weekNumber)` = last of `getWeekConfig(weekNumber).tasks`, **NOT** a hardcoded type list, so W2's `contradiction_report` closing task is covered (W1/W3/W4 close on `shift_report`). It lazily ensures a `ShiftResult` exists so a lost or never-reached `ShiftClosing` POST (e.g. the W4 Drop Box → Recruitment epilogue gating it) still registers the grade. The server aggregate **mirrors `frontend/src/utils/scoreAggregator.ts` — keep the two in sync.** Skips if `completedAt` already set (never clobbers the richer client write); flips a `completedAt:null` marker to complete; **omits `concernScoreDelta`** (client is the authority). Non-fatal — wrapped in try/catch after the score commits, never 500s the save. `ShiftResult` is pair-only (legacy User scores keep the MissionScore fallback).
- **Backfill**: `backend/src/scripts/backfillShiftResults.ts` (dry-run default; `--commit`; run with the prod `DATABASE_URL`) recovers already-stuck students — a complete final-task MissionScore but no `ShiftResult.completedAt`. Keys on each week's `getClosingTaskType`, so it recovers Shift 2 too. Cannot recover students who never finished the closing task.
...
- **Socket emissions go to the class STAFF room** (`classRoomForPair()` returns `class:${classId}:staff`): `student:remediation-fired` (on trigger creation), `student:remediation-completed` (on complete), `student:remediation-clawback` (on clawback) — never the plain class room (students would receive classmates' behavior data). Payload always includes `pairId`, `designation`, `moduleId`, `weekNumber`. Used by ClassMonitor live chip + Gradebook drill-down.
...
- **Harmony censure respond mastery is gated the same way** (2026-06-10) — `POST /censure-queue/:id/respond` still upserts on re-answer (the 7-day spaced-repetition clock is BY DESIGN), but the +0.05/+0.03 mastery bump only runs on the first answer or a re-answer ≥7 days after the prior one. Any new graded surface that allows re-answers needs this same gate or it's replay-farmable.
- **Compliance `/pending` serves the ARCHIVED question set** (2026-06-10) — `buildComplianceQuestions` is shuffled/non-deterministic and the upsert keeps the first-generated `questions` JSON, so the response must come from `record.questions`, not the freshly-built set, or a refreshed student answers different questions than the teacher's results drill-down shows.
- **ClassWeekUnlock gate lives on ALL MissionScore writers** (2026-06-10) — `/shifts/.../score`, `/shifts/progress/:weekId/:stepId`, `/sessions/.../phase/.../complete`, and `/submissions/evaluate` (which also validates the client-sent missionId exists). Any NEW endpoint that writes MissionScore must replicate the gate or it's a pre-credit path for locked weeks.
...
## Socket Server — role-split rooms + sync registration (reworked 2026-06-10/11; original per-class scoping PR #36/#38)
- **Teachers join `class:${classId}:staff` rooms, NOT the plain class room.** Students join `class:${classId}`. Every teacher-bound emit (live status relays via `emitToStudentClass`, remediation fired/completed/clawback, `teacher:clarity-reply`, `student:connected/disconnected/deleted/registered`, `teacher:pause-state`) targets the staff room — the plain class room reaches every student browser in the class, and these payloads carry minors' names/behavior data. Class-wide events students consume (`session:paused/resumed`, `session:gate-updated`, `session:shift-changed`, `class:presence`, `harmony:new-content`, `inscription:trial-scheduled/force-aborted`) stay on `class:${classId}`. Personal `teacher:${teacherId}` room unchanged.
- **Class creation must join the teacher's LIVE sockets to the new staff room** (`classes.ts` POST / does `io.in(\`teacher:${teacherId}\`).socketsJoin(...)`) — room membership is otherwise snapshotted at connect time, and a mid-session class had dead live-monitoring plus a pause-with-no-Resume trap.
- **The connection handler registers ALL `socket.on(...)` handlers SYNCHRONOUSLY**, with bodies gated on a shared `ready` promise that does the async enrollment/room setup. Socket.IO acks CONNECT before the handler's awaits finish and silently DROPS events with no registered listener — the old register-after-await shape lost the clients' connect-time emits under reconnect storms. `disconnect` also awaits `ready` (a socket flapping inside the setup window otherwise leaked its id into `entitySockets` forever → student shown perpetually online).
- **`classId` resolves via `prisma.classEnrollment.findFirst({ where: { OR: [{ pairId }, { userId }] } })`** — captured BEFORE cascade-delete in delete handlers.
...
- **On-topic veto rubric** (post 2026-04-29; grading stabilised 2026-06-22): `(1) onTopic boolean` — VETO axis, **defaults true / lenient**, graded **deterministically at `temperature: 0`**; `(2) vocabScore (0–1)` — only numeric axis; `(3) grammarAdvisory string` — non-scoring, teacher-only. Recorded score = `onTopic ? clamp(vocabScore, 0.1, 1) : 0`; **pass = `onTopic && vocabScore >= 0.3`** (was 0.4). Off-topic caps the score but **no longer hard-locks completion** — the frontend's one-nudge-then-escapable flow guarantees a way through. Vocab is the SCORED axis, **not a gate** (the 30%-of-targets Layer-1 hard block was removed 2026-06-22). Do NOT re-add a hard off-topic lock or hard vocab floor.
...
## Startup Migration: Dictionary Words for All Weeks (added 2026-06-09)
- **`backend/src/utils/weekConfigMigrations.ts::ensureDictionaryWordsForAllWeeks()`** scans every `Week`, looks up its `WeekConfig`, and inserts a missing `DictionaryWord` row for each `targetWords` entry using authored enrichment from **`backend/src/data/week-configs/wordEnrichment.ts`**. Wired fire-and-forget in `index.ts` alongside `ensureQueueMissionsForAllWeeks`.
- **Why**: `prisma/seed.ts` is the ONLY other writer of `DictionaryWord`, it stops at week 3, and it does NOT run on Railway (only `prisma migrate deploy` does). So W4+ target words had no row → Shift-Report writing never bumped mastery, `buildComplianceQuestions` did `if (!row) continue` (teachers couldn't author a Compliance Check on the real targets), and Clarity/Remediation mastery bumps no-op'd.
- **Idempotent + create-only**: `findFirst({ word, weekIntroduced }) → skip if exists`, so it NEVER overwrites the richer W1–3 seed rows. Words without authored enrichment are skipped (a hollow empty-`definition` row is useless to Compliance).
- **Adding a new shift now requires authoring its `targetWords` enrichment in `wordEnrichment.ts`** (definition / phonetic / `translationZhTw` / example / `toeicCategory`). The 5 W4 "Black Words" are deliberately ABSENT — they're narrative B1 vocab, not TOEIC targets, and must stay out of the mastery/Compliance/Remediation pipeline (they're not in `targetWords` either, so they're excluded by construction).
...
## Audit Batches 4-6 Invariants (added 2026-06-10/11)
- **`utils/moveEpochs.ts`** — per-pair in-memory epoch bumped by move-to-shift + every task-command (`skip/reset-task/reset-shift/send-to-task`). `/submissions/evaluate` snapshots it BEFORE its OpenAI round-trip and skips MissionScore + mastery writes on mismatch. Any new long-running writer (anything that awaits an external service before writing pair progress) must do the same, or an in-flight request re-creates rows a teacher command just deleted.
- **Replay policy** — reset-shift, per-week progress DELETE, and all three `moveOnePairToShift` branches clear the affected week's `CharacterMessage` rows **excluding `triggerType: 'direct_message'`** (Clarity Minder threads share the table — never wipe them) so story beats re-fire on replay. `NarrativeChoice` + `Citizen4488Interaction` are kept BY DESIGN (votes not re-cast, spy dice not re-rolled, Harmony never reset by shift commands). Frontend re-syncs `loadMessages()` after reset/move or the in-memory dedup keeps suppressing the re-fires.
- **Anchors**: reset-shift + the per-week DELETE re-create the `completedAt: null` marker (conditional on it being the pair's current week for the DELETE); send-to-task + `DELETE /scores/:scoreId` (when the row is shift_report/clock_out) CONVERT the ShiftResult to a marker via `updateMany completedAt→null` — never delete it, the anchor and recorded concernScoreDelta must survive a partial redo.
- **Score writes validate finite [0,1]** with a 400 — teacher `PATCH /scores/:scoreId` AND student `POST /shifts/.../score`.
- **Mastery is verified-surface-only**: the dictionary + legacy vocabulary `/encounter` routes bump `encounters`/`lastSeenAt` ONLY (one-time 0.1 first-encounter seed is sanctioned). The writing-path bump is gated on `aiResult.onTopic`. Any new mastery writer needs a verification gate + replay protection.
- **task-command iterates missions in WeekConfig task order** (map via `missionType`), never `orderBy orderIndex` — prod rows from expanded configs had stale/colliding indices (W1-3 had cloze_fill↔vocab_clearance transposed). `ensureQueueMissionsForAllWeeks` also heals drifted `orderIndex` values on boot.
- **`GET /clarity-check/completed?weekNumber=`** — serves the pair's completed checkIds so the frontend one-shot gate survives refresh (ShiftQueue hydrates it before evaluating placements).
- **Layer-1 writing fail is word-count ONLY** (the 30%-of-targets vocab block was removed 2026-06-22 — vocab is now scored, not gated) and carries `onTopic: true` (a length verdict is not a topic verdict); `/evaluate` honors `metadata.minWordCount` (lane-resolved per-task floor, clamped 5-100) as the single writing floor.
- **Inscription reconnect**: `inscription:enter-drill` also joins the lobby room and replays paused/resumed state from DB; queue eviction notifies `student:${pairId}` with `queue-error: queue_dropped`; the join path dedups by pairId (refreshes socketId) so client re-joins on 'connect' are idempotent.
- **Deferred with cause (do not re-find)**: `Mission.config` read-modify-write races in the briefing/step-video routes (ms-window, single-teacher ops); `inscription:drill-complete` has no consumer yet (kept for a live final-standings wire-up); legacy `vocabulary.ts` + unmounted `SafeProperApp` deletion.
